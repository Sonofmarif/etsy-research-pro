...
async function saveFinalResults(seedKeyword, step4Result, config) {
  ...
  const scoredListings = listings.map(l => {
    ...
    const shopReviews = parseInt(l.shop_reviews || l.review_count || 0);
    const inCarts = audit ? parseInt(audit.in_carts || 0) : 0;
    const sold24h = audit ? parseInt(audit.sold_24h || 0) : 0;
    
    let score = 50; 
    if (shopReviews < (config.max_shop_reviews_beatable || 300)) score += 20;
...
// ─── Log & History persistence helpers ───────────────────────────────
async function flushLogsToServer(seedKeyword, step) {
  try {
    ...
    // We can sync logs to telemetry route or report-error if failed
    ...
    if (errorLogs.length > 0) {
      await fetch(`${baseUrl}/report-error`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          error_message: `Pipeline Fail: ${seedKeyword} - ${step}`,
          stack_trace: JSON.stringify(errorLogs),
